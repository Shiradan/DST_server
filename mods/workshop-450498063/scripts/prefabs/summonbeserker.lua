local brain = require "brains/summonbeserkerbrain"

local assets = {
	Asset("ATLAS", "images/inventoryimages/summonbeserker.xml"),
...
local function fn()
    ...
    inst.MiniMapEntity:SetIcon("summonbeserker.tex")
    inst.MiniMapEntity:SetPriority(4)
	
	inst:AddTag("summonbeserker")
	inst:AddTag("sheltercarrier")
	inst:AddTag("summonedbyplayer")
	inst:AddTag("scarytoprey")
	inst:AddTag("illyaspecific")
	...
	inst:AddComponent("beserkercracker")
	...
	inst.components.health:SetMaxHealth(7500)
	...
	inst.sword = SpawnPrefab( "beserkersword" )
	...
	inst:SetBrain(brain)
	inst:SetStateGraph("SGsummonbeserker")
	
	inst:AddComponent("perishable")
	inst.components.perishable:SetPerishTime(1.5*30*16*1)
	inst.components.perishable:StartPerishing()
	inst.components.perishable.onperishreplacement = "grail"
...
return Prefab("common/summonbeserker", fn, assets, prefabs)