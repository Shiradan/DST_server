...
local prefabs =
{
	...
	if owner.prefab == "illya_mahoo" then
		...
		owner.Mag_Power2 = owner.Mag_Power2 + 15
		end
...
local function onlightning(staff, caster, target)
	if caster.components.sanity ~= nil and caster.components.sanity.current >= 40 then
		caster.components.sanity:DoDelta(- 40)
		else
		caster.components.health:DoDelta(- 10, true)
		end
...
return Prefab("common/inventory/illya_rubystar", fn, assets, prefabs)
