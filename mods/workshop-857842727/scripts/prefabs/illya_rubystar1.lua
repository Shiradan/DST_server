...
local prefabs =
{
	...
	if owner.prefab == "illya_mahoo" then
		...
		owner.Mag_Power2 = owner.Mag_Power2 + 40
		end
...
local function onlightning(staff, caster, target)
	if caster.components.sanity ~= nil and caster.components.sanity.current >= 45 then
		caster.components.sanity:DoDelta(- 45)
				else
		caster.components.health:DoDelta(- 12, true)
		end
...
return Prefab("common/inventory/illya_rubystar1", fn, assets, prefabs)
