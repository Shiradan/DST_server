...
local prefabs =
{
	...
	if owner.prefab == "illya_mahoo" then
		...
		owner.Mag_Power2 = owner.Mag_Power2 + 100
		end
	else
...
local function onlightning(staff, caster, target)
	if caster.components.sanity ~= nil and caster.components.sanity.current >= 60 then
		caster.components.sanity:DoDelta(- 60)
			else
		caster.components.health:DoDelta(- 14, true)
		end
...
return Prefab("common/inventory/illya_rubystar3", fn, assets, prefabs)
