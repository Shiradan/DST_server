...
local function CryForQt(inst)
	inst.components.talker:Say("Atleast I'm away from her.")
	for k, v in pairs(AllPlayers) do
		if v.prefab == "saberlion" then
			if not v.components.leader then
				v:AddComponent("leader")
			end
			v.components.leader:AddFollower(inst, true)
			inst:DoTaskInTime(2, function()
				inst.components.talker:Say("Looks like I'm wrong...")
			end)
			break
		end
	end
...
local SummonAssassin = Class(Brain, function(self, inst)
	Brain._ctor(self, inst)
end)

function SummonAssassin:OnStart()
